...
| 7 External | **EHAS2 canonical contract:** [rules/rule-07-external-use-routes-contract.md](./rules/rule-07-external-use-routes-contract.md) (`EXTERNAL_USE_ROUTES`, owner-locked, `R7_ID01_TO_ID05_RECOMMENDED_DECISIONS_APPROVED`). Legacy row (reference only): `external_application_engine.build_external_routes` | Canonical shadow inputs per contract; legacy: systems, symptoms, BP, phase, CC | Shadow route/site indication or `NOT_CLINICALLY_INDICATED`; legacy: routes | **Shadow-only** until separate impl/orchestration auth; clinical activation **NONE**; no external medicine/instructions in this stage; legacy = LEGACY_REFERENCE_ONLY | No oral→external copy; no legacy fill/replace in EHAS2 | Contract proof matrix pending impl | EHAS2: identity/scope/contract documented · impl **NOT_AUTHORIZED** · fail-closed · independent of Rule 5 C3E and Rule 6 relationship data |
