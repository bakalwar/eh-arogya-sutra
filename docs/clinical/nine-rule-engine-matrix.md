...
**EHAS2 Rule 3 authority:** identity/scope owner-locked (`ORGAN_SYSTEM_AFFINITY`); focused canonical contract: [rules/rule-03-organ-system-affinity-contract.md](./rules/rule-03-organ-system-affinity-contract.md) (`R3_ID01_TO_ID05_RECOMMENDED_DECISIONS_APPROVED`). Package `@ehas2/rule3` / evaluator **absent** · **NOT_IMPLEMENTED** / **NOT_AUTHORIZED**. Mappings **0** · catalog **NOT_CREATED** · no closed real organ-system catalog · empty registry fail-closed · medicine influence **NONE** · formula mutation **NONE** · Rule 4 **not** connected · orch **NOT_CONNECTED** · activation/Rx **NONE**. Frozen clinical body: [rules/rule-03-organ-system-affinity.md](./rules/rule-03-organ-system-affinity.md). Historical matrix rows describing legacy keyword / METABOLIC fallback remain **LEGACY_REFERENCE_ONLY** and do not override this contract. Interface stub `affectsClinicalSelection: true` is **metadata drift, not authority**.
...
| 3 Organ-System Affinity Engine | **EHAS2 canonical contract:** [rules/rule-03-organ-system-affinity-contract.md](./rules/rule-03-organ-system-affinity-contract.md) (`ORGAN_SYSTEM_AFFINITY`, owner-locked, `R3_ID01_TO_ID05_RECOMMENDED_DECISIONS_APPROVED`). Package `@ehas2/rule3` **absent** · evaluator **NOT_IMPLEMENTED** / **NOT_AUTHORIZED**. Frozen clinical spec (body preserved): [rules/rule-03-organ-system-affinity.md](./rules/rule-03-organ-system-affinity.md). Legacy row (reference only): `integrated_engine.detect_systems` / `detect_active_systems` | Canonical shadow inputs per contract; legacy: symptoms, gender, CC | Shadow organ/system annotations / fail-closed outcomes per contract; legacy: systems + details | **Shadow-only** (future); medicine-selection influence **NONE**; formula mutation **NONE**; Rule 4 **not** connected; orchestration **NOT_CONNECTED**; clinical activation **NONE**; prescription effect **NONE**; real mappings **0**; catalog **NOT_CREATED**; no closed real organ-system catalog | Empty registry fail-closed; **no** automatic METABOLIC fallback; unresolved → empty active + `INSUFFICIENT_ORGAN_SYSTEM_EVIDENCE` | P01–P18 documented, **not executed** (contract tranche) | EHAS2: identity/scope/contract documented · package absent · empty registry fail-closed · independent of Rule 5 C3E; legacy “plans + boosts” = LEGACY_REFERENCE_ONLY |
