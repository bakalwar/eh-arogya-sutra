...
| 8 Disease Prakruti | **EHAS2 canonical contract:** [rules/rule-08-disease-level-prakruti-inference-contract.md](./rules/rule-08-disease-level-prakruti-inference-contract.md) (`DISEASE_LEVEL_PRAKRUTI_INFERENCE`, owner-locked, `R8_ID01_TO_ID05_RECOMMENDED_DECISIONS_APPROVED`). Legacy row (reference only): `infer_disease_prakruti` / `resolve_prakriti` | Canonical shadow inputs per contract; legacy: disease name / DB votes | Shadow disease-level prakriti indication or `NOT_CLINICALLY_INDICATED`; legacy: temperament label | **Shadow-only** until separate impl/orchestration auth; clinical activation **NONE**; medicine-selection influence **NONE**; `NOT_REQUIRED_FOR_PRESCRIPTION`; real approved mappings **0**; legacy = LEGACY_REFERENCE_ONLY | No Rule 1 overwrite/silent merge; no medicine/formula authority | Contract proof matrix pending impl | EHAS2: identity/scope/contract documented · impl **NOT_AUTHORIZED** · fail-closed · separate from Rule 1 · independent of Rule 5 C3E / Rule 6 edges / Rule 7 mappings |
...
- whether Rule 8 becomes **clinically activated** (identity/scope now owner-locked; shadow contract documented; activation remains separate)  
