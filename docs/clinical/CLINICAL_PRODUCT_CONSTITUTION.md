...
Rule 9 Master Pipeline canonical contract (`MASTER_PIPELINE`; validate/reject/package-only; **no** impl): [rules/rule-09-master-pipeline-contract.md](./rules/rule-09-master-pipeline-contract.md) (`R9_ID01_TO_ID05_RECOMMENDED_DECISIONS_APPROVED`; orchestration **NOT_CONNECTED**; clinical activation **NONE**; prescription effect **NONE**; no new medicine selection; no composition mutation; complexity tier **not** inferred by Rule 9).
