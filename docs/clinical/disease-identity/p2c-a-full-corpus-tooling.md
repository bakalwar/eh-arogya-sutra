...
- Controlled-index WAL lifecycle: builder-owned temporary SQLite only; `wal_checkpoint(TRUNCATE)` runs only at closed reader boundaries (after input ingest, after each 256-row mapped/bridge and disease output batch, after output population, and before final compact assertion). Checkpoint return values (`busy`, `log`, `checkpointed`) are inspected and fail closed on busy/incomplete results. Conservative headroom: `CONTROLLED_INDEX_CHECKPOINT_HEADROOM_BYTES` (64 MiB below cap). Monotonic peak bytes are never reset. Windows cleanup uses bounded ENOTEMPTY/EPERM/EBUSY retry without masking the primary structured error.
