...
- Controlled-index WAL lifecycle: builder-owned temporary SQLite only; `wal_checkpoint(TRUNCATE)` runs only at closed reader boundaries. **Unconditional** checkpoints: after each closed 256-row mapped/bridge output batch (`*-batch`), after disease output batches, `after-mapped-bridge-output`, `after-output-population`, and `final-compact-assertion`. **Threshold-gated only:** `after-input-ingest` runs when aggregate footprint ≥ cap minus `CONTROLLED_INDEX_CHECKPOINT_HEADROOM_BYTES` (64 MiB). Checkpoint return values (`busy`, `log`, `checkpointed`) are inspected and fail closed on busy/incomplete results. Monotonic peak bytes are never reset. Windows cleanup uses bounded ENOTEMPTY/EPERM/EBUSY retry with builder-issued ownership validation, without masking the primary structured error.
