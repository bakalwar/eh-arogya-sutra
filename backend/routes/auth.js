  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res.status(401).json({
        success: false,
        message: 'Account not found. Contact admin to register your mobile number.'
      });
    }
    ...
    if (!ok) {
      ...
      const remaining = Math.max(0, MAX_FAILED - ((row.failed_login_attempts || 0) + 1));
      return res.status(401).json({
        success: false,
        message:
          remaining > 0
            ? `Invalid mobile or password. ${remaining} attempt(s) left.`
