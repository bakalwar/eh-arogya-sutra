...
type CaduceusLogoProps = SVGProps<SVGSVGElement> & {
  size?: number;
  variant?: 'splash' | 'compact';
  gradientPrefix?: string;
};

export default function CaduceusLogo({
  size = 36,
  variant = 'compact',
  gradientPrefix = 'logo',
  ...props
}: CaduceusLogoProps) {
  const gid1 = `${gradientPrefix}-staff`;
  const gid2 = `${gradientPrefix}-wings`;
  const gid3 = `${gradientPrefix}-snakes`;
  ...
  if (variant === 'splash') {
    return (
      <svg
        ...
        viewBox="0 0 100 100"
        fill="none"
        style={{ filter: 'drop-shadow(0 0 18px rgba(201,150,58,0.4))' }}
        {...props}
      >
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" {...props}>
      <circle cx="50" cy="50" r="46" stroke="#c9963a" strokeWidth="1.5" opacity="0.5" />
      <line x1="50" y1="16" x2="50" y2="84" stroke="#c9963a" strokeWidth="3" strokeLinecap="round" />
      <path d="M50 24 C41 17 26 19 23 26 C30 22 42 25 50 31" fill="#e8c46a" opacity="0.9" />
      <path d="M50 24 C59 17 74 19 77 26 C70 22 58 25 50 31" fill="#e8c46a" opacity="0.9" />
      <path d="M50 31 C43 37 39 44 42 51 C45 57 42 64 37 70" stroke="#4a9b54" strokeWidth="2" fill="none" strokeLinecap="round" />
      <path d="M50 31 C57 37 61 44 58 51 C55 57 58 64 63 70" stroke="#4a9b54" strokeWidth="2" fill="none" strokeLinecap="round" />
      <circle cx="50" cy="50" r="3" fill="#e8c46a" />
    </svg>
  );
}
