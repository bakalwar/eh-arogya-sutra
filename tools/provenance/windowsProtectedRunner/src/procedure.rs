//! Provenance: adapted from C3C windowsHandleSpike/src/procedure.rs (PR #78; B1 ADS parser, handle walk).
...
use crate::outcome::RunnerResult;
...
impl OwnedHandle {
    fn new(h: HANDLE) -> Result<Self, RunnerCode> {
        if h.is_null() || h == INVALID_HANDLE_VALUE {
            Err(RunnerCode::Internal)
        } else {
            Ok(Self(h))
        }
    }
...
pub fn plan_component_walk(selection: &str) -> Result<PathPlan, RunnerCode> {
    validate_external_selection(selection)
}

pub fn run_windows_runner(selection: &str) -> RunnerResult {
    match run_windows_runner_inner(selection) {
        Ok(()) => RunnerResult::ok(),
        Err(code) => RunnerResult::failed(code),
    }
}

fn run_windows_runner_inner(selection: &str) -> Result<(), RunnerCode> {
    ...
    if size > MAX_ACCEPTED_BYTES {
        return Err(RunnerCode::Oversize);
    }
...
fn verify_volume(plan: &PathPlan) -> Result<(), RunnerCode> {
    let root = format!("{}:\\", plan.drive);
    let obs = collect_volume_observation(&root)?;
    classify_volume_observation(&obs)
}

pub fn collect_volume_observation(root_path: &str) -> Result<VolumeObservation, RunnerCode> {
...
fn open_via_component_walk(plan: &PathPlan) -> Result<(OwnedHandle, WalkEvidence), RunnerCode> {
...
fn open_volume_root(drive: char) -> Result<OwnedHandle, RunnerCode> {
    let nt = format!("\\??\\{}:\\", drive);
...
fn open_relative(parent: HANDLE, component: &str, is_final_file: bool) -> Result<OwnedHandle, RunnerCode> {
    ...
    if is_reparse_handle(owned.raw())? {
        return Err(RunnerCode::ReparseRejected);
    }
    if !is_final_file && !is_directory_handle(owned.raw())? {
        return Err(RunnerCode::Internal);
    }
    Ok(owned)
}

fn is_reparse_handle(handle: HANDLE) -> Result<bool, RunnerCode> {
...
fn is_directory_handle(handle: HANDLE) -> Result<bool, RunnerCode> {
...
fn map_open_status(status: NTSTATUS) -> Result<(), RunnerCode> {
...
fn verify_final_file_metadata(handle: HANDLE) -> Result<(), RunnerCode> {
    let mut stdinfo: FILE_STANDARD_INFO = unsafe { zeroed() };
    let ok = unsafe {
        GetFileInformationByHandleEx(
            handle,
    ...
    if stdinfo.NumberOfLinks != 1 {
        return Err(RunnerCode::HardlinkRejected);
    }
...
fn verify_default_unnamed_stream_only(handle: HANDLE) -> Result<(), RunnerCode> {
    ...
    if ok == 0 {
        ...
        return Err(RunnerCode::AdsRejected);
    }
...
pub(crate) fn parse_file_stream_info_default_only(buf: &[u8]) -> Result<(), RunnerCode> {
    ...
    loop {
        ...
        let name_len = usize::try_from(stream_name_length_u32).map_err(|_| RunnerCode::AdsRejected)?;
        if name_len % 2 != 0 {
            return Err(RunnerCode::AdsRejected);
        }
        ...
        if next_entry_offset != 0 {
            let next_rel =
                usize::try_from(next_entry_offset).map_err(|_| RunnerCode::AdsRejected)?;
        ...
        let next_rel = usize::try_from(next_entry_offset).map_err(|_| RunnerCode::AdsRejected)?;
...
pub fn capture_identity(handle: HANDLE) -> Result<IdentityTuple, RunnerCode> {
    ...
    if ok == 0 {
        return Err(RunnerCode::IdentityUnavailable);
    }
    let mut stdinfo: FILE_STANDARD_INFO = unsafe { zeroed() };
    let ok2 = unsafe {
        GetFileInformationByHandleEx(
...
fn end_of_file(handle: HANDLE) -> Result<u64, RunnerCode> {
    let mut stdinfo: FILE_STANDARD_INFO = unsafe { zeroed() };
    let ok = unsafe {
        GetFileInformationByHandleEx(
            handle,
    ...
    if ok == 0 {
        return Err(RunnerCode::ReadFailed);
    }
...
pub fn open_for_share_test(selection: &str) -> Result<(OwnedHandle, IdentityTuple), RunnerCode> {
    require_windows11_x64()?;
    let plan = validate_external_selection(selection)?;
    verify_volume(&plan)?;
...
pub fn walk_evidence_for_selection(selection: &str) -> Result<WalkEvidence, RunnerCode> {
    require_windows11_x64()?;
    let plan = validate_external_selection(selection)?;
    verify_volume(&plan)?;
...
fn to_wide_null(s: &str) -> Result<Vec<u16>, RunnerCode> {
...
pub type RunnerOwnedHandle = OwnedHandle;
...
mod stream_info_bounds_tests {
    ...
    fn assert_ads(err: Result<(), RunnerCode>) {
        assert_eq!(err, Err(RunnerCode::AdsRejected));
        let msg = format!("{err:?}");
